...
%% Plot the image and the SIFT features for 3 different threshold level
clf;

[frames1, descrs1] = getFeatures(im1, 'peakThreshold', 0.001) ;
figure(1) ;
set(gcf,'name', 'Part I.A: SIFT features detection - synthetic pair') ;
imagesc(im1) ; axis equal off ; hold on ;
vl_plotframe(frames1, 'linewidth', 2) ;

[frames1, descrs1] = getFeatures(im1, 'peakThreshold', 0.004) ;
figure(2) ;
set(gcf,'name', 'Part I.A: SIFT features detection - synthetic pair') ;
imagesc(im1) ; axis equal off ; hold on ;
vl_plotframe(frames1, 'linewidth', 2) ;
...
%% Plot the two images for the same threshold
clf;
[frames1, descrs1] = getFeatures(im1, 'peakThreshold', 0.001) ;
figure(1) ;
set(gcf,'name', 'Part I.A: SIFT features detection - synthetic pair') ;
...
figure(2) ;
set(gcf,'name', 'Part I.A: SIFT features detection - synthetic pair') ;
imagesc(im2) ; axis equal off ; hold on ;
vl_plotframe(frames2, 'linewidth', 2) ;
...
% saveas(gcf, sprintf('IC1-ratio%d.png',10*nnThreshold), 'png') ;
